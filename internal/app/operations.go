...
func (a *App) Trip(ctx context.Context, reason string) error {
	// A batch abort must halt every in-flight dosepump ramp loop; otherwise the
	// pump opening keeps climbing (and the cumulative fill reading creeps up)
	// even after the conveyor has stopped. cancelAllSterileLoops tears down any
	// goroutine that is still bumping SterileFlowTPH.
